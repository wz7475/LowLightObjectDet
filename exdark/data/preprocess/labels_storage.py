...
exdark_coco_like_labels = [
    exdark2coco_exceptions.get(label, label) for label in exdark_custon_labels
]
exdark_idx2label = dict(
    (idx, label) for idx, label in enumerate(exdark_coco_like_labels)
)
exdark_label2idx = dict((v, k) for k, v in exdark_idx2label.items())

coco2coco_like_exdark = dict(
    (coco_labels.index(category_name), idx)
    for idx, category_name in enumerate(exdark_coco_like_labels)
)
