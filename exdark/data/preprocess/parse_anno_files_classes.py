...
def parse_exdark_anno_dir_flat(input_dir: str, output_dir: str, labels_map: dict):
    ...
    for anno_file in os.listdir(input_dir):
        ...
        anno_df = pd.read_csv(
            ...
            delimiter=" ",
            header=None,
            names=anno_file_columns,
        )
        anno_df["class"] = anno_df["class"].map(
            lambda class_name: labels_map[class_name.lower()]
        )
        anno_df["class"] = anno_df["class"].map(
            lambda class_name: exdark_label2idx[class_name.lower()]
        )
...
if __name__ == "__main__":
    ...
    _, labels_names_map = generate_exdark2coco_mapping(
        coco_labels, exdark_custon_labels
    )
